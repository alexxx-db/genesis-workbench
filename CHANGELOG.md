...
- **Vortex (AI Canvas) palette shows module batch workflows as "not deployed" even though their jobs are deployed** (e.g. GWAS, VCF Ingestion, Variant Annotation, Variant Calling, Fine-Tune ESM2 all read "not deployed" while their Databricks Jobs exist and run fine).
  ...
  **Proper fix (upstream, TODO):** run `grant_app_permissions` at the end of *each module* deploy (not just core), and/or give `_all_job_names_cache` a TTL or invalidate it on catalog refresh; alternatively register module batch workflows in `batch_models` so the palette doesn't depend on the SP-visible `jobs.list()`.
  **Related data-hygiene:** deleting jobs out-of-band leaves stale `*_job_id` values in the `settings` table too (not just `batch_models`) — reconcile **both** after a job dedup (`SELECT key,value FROM settings WHERE key LIKE '%_job_id'`).
