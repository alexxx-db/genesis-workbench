...
# 3-step progress chip — emoji-encoded status for the single-cell pipeline.
PROGRESS_MAP = {
    "started": "🟩⬜⬜",
    "processing": "🟩🟩⬜",
    "complete": "🟩🟩🟩",
...
def download_cluster_markers_mapping(run_id: str) -> pd.DataFrame:
    ...
    discriminative first). Enrichment uses
...
#
# Uses MlflowClient.log_artifact directly (not mlflow.start_run +
...
# Storage format kept stable so existing saved annotations from earlier
# pipeline runs stay loadable on the same runs:
